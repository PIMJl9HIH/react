...
const Category = ({
  ...
  <div>
    <div className="post__category-container">
      ...
      <div className="postAddingCategory category__list"
           style={{display: posts[curPost].visibilityAddingCategory ? 'block' : 'none'}}>
        {/*<FormGroup>*/}
        ...
        <ListGroup>
          {missingCategory(curPost).map((item, index) =>
            <ListGroupItem className="categoryItem" tag="a" href="#" action key={index}
                           onClick={(event) => addCategory(curPost, posts, event)}>{item} </ListGroupItem>
          )}
        </ListGroup>
      </div>
